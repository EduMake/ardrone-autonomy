...
function Controller(client, options) {
    ...
    this._alwayscallback = options.alwayscallback || false;
	this._simulation = options.simulation || false;
	this._relativetogoal = options.relativetogoal || false;
 ...
 * In simulation mode assume we have reached the goal
 */
Controller.prototype.state = function() {
	if(this._simulation) {
		if(this._goal == null){ 
			this._goal = {x: 0, y: 0, z: 1, yaw: 0}
		}
		if(typeof this._goal.yaw =="undefined" || this._goal.yaw == null){
			this._goal.yaw = this._state.yaw;
		}
		this._state = this._goal ;
	}
    return this._state;
}
...
/*
 * Return the drone basestate (x,y,z,yaw) as estimated
 * by the Kalman Filter 
 * or if this._relativetogoal based on the goal
 * In simulation mode assume we have reached the goal
 */
Controller.prototype.basestate = function() {
	if(this._relativetogoal || this._simulation) {
		if(this._goal == null){ 
			this._goal = {x: 0, y: 0, z: 1, yaw: 0}
		}
		if(typeof this._goal.yaw =="undefined" || this._goal.yaw == null){
 ...
 */
Controller.prototype.hover = function(dist, callback) {
	var state = this.basestate();
    return this._go({x: state.x, y: state.y, z: state.z, yaw: state.yaw}, callback);
}
...
Controller.prototype.move = function(move, callback) {
   // Our starting position
   var state = this.basestate();
   ...
   // Remap our target position in the world coordinates not sure the maths is correct
	var gx = state.x + (Math.cos(state.yaw) * move.x)  - (Math.sin(state.yaw) * move.y);
	var gy = state.y + (Math.sin(state.yaw) * move.x)  + (Math.cos(state.yaw) * move.y);
	var gz = state.z + move.z

   // Assign the new goal
   this._go({x: gx, y: gy, z: gz, yaw: state.yaw}, callback);
}
 ...
 * Sets a new goal and enable the controller. When the goal
 * is reached, the callback is called with the current state.
 * if this._alwayscallback then calls back also when goal abandoned
 * x,y,z in meters
 * yaw in rads
 */
Controller.prototype._go = function(goal, callback) {
    ...
    if (this._alwayscallback && this._callback != null) {
